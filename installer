...
/**
 * check the platform for possible issues on running gush
 */
function checkPlatform($quiet)
{
    ...
    if (!empty($errors)) {
        out("Some settings on your machine make Gush unable to work properly.", 'error');
    ...
    if (!empty($warnings)) {
        out("Some settings on your machine may cause stability issues with Gush.", 'error');
        ...
        foreach ($warnings as $warning => $current) {
            switch ($warning) {
                ...
                case 'openssl':
                    $text = PHP_EOL."The openssl extension is missing, which will reduce the security and stability of Gush.".PHP_EOL;
                    $text .= "If possible you should enable it or recompile php with --with-openssl";
                    break;

                case 'php':
                    $text = PHP_EOL."Your PHP ({$current}) is quite old, upgrading to PHP 5.4 or higher is recommended.".PHP_EOL;
                    $text .= "Gush works with 5.4+ for most people, but there might be edge case issues.";
                    break;
            }
            if ($displayIniMessage) {
                $text .= $iniMessage;
...
/**
 * installs gush to the current working directory
 */
function installGush($installDir, $quiet)
{
    $installPath = (is_dir($installDir) ? rtrim($installDir, '/').'/' : '') . 'gush.phar';
    $installDir = realpath($installDir) ? realpath($installDir) : getcwd();
    $file       = $installDir.DIRECTORY_SEPARATOR.'gush.phar';
    ...
    if (!$quiet) {
        out(PHP_EOL."Gush successfully installed to: " . $file, 'success', false);
